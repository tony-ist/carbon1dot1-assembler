use crate::{assembler::assemble, ast::FuncBody, lower_labels::lower_labels, name_mangling::mangle, parser::parse_input_or_emit_error_and_exit, preprocessing::preprocess_defines};
...
pub fn assemble_source_or_emit_error_and_exit<'a>(source: &'a str) -> Vec<u8> {
    ...
    let disassembled = disassemble(&lowered); // todo print
    let assembled = assemble(&lowered);
    assembled
}

// todo implement and move to disassembler.rs 
